...
t60 = hslider("Decay [unit:seconds] [OWL:PARAMETER_C]", 4, 0.1, 120, 0.01) : smooth(tau2pole(smooth_time));
mix = hslider("Mix [OWL:PARAMETER_D]", 0.5, 0, 1, 0.01) : smooth(tau2pole(smooth_time));
damp = 0.3;
...
f = midikey2hz(coarse_pitch+fine_pitch) : smooth(tau2pole(smooth_time));

string(x, s) = (+ : fdelay(dtmax, dtsamples)) ~ (*(fbk))
//string(x, s) = (+ : fdelay(dtmax, dtsamples)) ~ (dampingfilter : *(fbk))
with {
	thisFreq = f * ratios(s);
	dtsamples = (SR/thisFreq) - 2;
